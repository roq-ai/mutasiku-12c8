...
import { getMutationData } from 'apiSdk/mutation-data';
import { MutationDataInterface } from 'interfaces/mutation-data';
import { getUserInvitations } from 'apiSdk/user-invitations';
...
import { MutationDataListPage as ShowCaseList } from 'pages/mutation-data';

function DashboardPage() {
  const { session } = useSession();

  const mutationDataStat = useSWR<PaginatedInterface<MutationDataInterface>>(
    () => `/mutation-data`,
    () => getMutationData({ limit: 1, offset: 0 }),
  );

  const userInvitationStat = useSWR<PaginatedInterface<UserInvitationInterface>>(
    () => `/user-invitations`,
    () => getUserInvitations({ limit: 1, offset: 0 }),
  );

  const bankAccountStat = useSWR<PaginatedInterface<BankAccountInterface>>(
    () => `/bank-accounts`,
    () => getBankAccounts({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Mutation Data',
      result: mutationDataStat?.data?.totalCount,
      isLoading: mutationDataStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'User Invitations',
      result: userInvitationStat?.data?.totalCount,
      isLoading: userInvitationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Bank Accounts',
      result: bankAccountStat?.data?.totalCount,
      isLoading: bankAccountStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`mutation-data`} passHref legacyBehavior>
                  <Button
