...
import { getAccountBalanceById, updateAccountBalanceById } from 'apiSdk/account-balances';
...
function AccountBalanceEditPage() {
  ...
  const handleSubmit = async (values: AccountBalanceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateAccountBalanceById(id, values);
      mutate(updated);
      resetForm();
