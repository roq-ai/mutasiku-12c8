...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'financial-manager', 'accountant', 'customer-service-agent'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<OnlineSellerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/online-sellers` : null),
    () => getOnlineSellers({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Online Sellers',
      path: isTenantUser ? `/online-sellers/view/${data?.[0]?.id}` : '/online-sellers',
