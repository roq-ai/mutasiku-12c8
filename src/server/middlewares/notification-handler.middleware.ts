...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'bank_account.create': {
    roles: ['business-owner', 'financial-manager', 'accountant', 'customer-service-agent'],
    key: 'bank-account-added',
    tenantPath: ['online_seller', 'bank_account'],
    userPath: [],
  },
  'bank_account.delete': {
    roles: ['business-owner', 'financial-manager', 'accountant', 'customer-service-agent'],
    key: 'bank-account-removed',
    tenantPath: ['online_seller', 'bank_account'],
    userPath: [],
  },
  'bank_account.update': {
    roles: ['business-owner', 'financial-manager', 'accountant', 'customer-service-agent'],
    key: 'bank-account-updated',
    tenantPath: ['online_seller', 'bank_account'],
    userPath: [],
  },
};
...
const tenantRoles: string[] = ['business-owner', 'financial-manager', 'accountant', 'customer-service-agent'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.online_seller.findFirst({
